...
async function lastOfferMail(nft, offerer, amount,lastOffer){  
    ...
    if(typeof(nftData) == 'object' && typeof(lastOfferData)=='object'){
        if(lastOfferData.notification && 'items' in (JSON.parse(lastOfferData.notification)) && (JSON.parse(lastOfferData.notification)).items==true){
            let htmlContent = "<p>Hi "+lastOfferData.displayName+",</p><p>Your offer rejected on NFT <b><a href='"+(JSON.parse(nftData.ipfs)).image+"' target='_blank'>"+(JSON.parse(nftData.ipfs)).name+"</a></b> and winning offer is <b>"+Number(amount)/10**18+"</b> <img src='https://rest-api-oneto11-dev.herokuapp.com/currency/curr_logo.png' style='width:18px;height:18px; position:relative;top:-2px; margin-right:2px;' alt = ''/></p>";
            htmlContent = htmlContent + "<p>Winner name is: <b>"+userData.displayName+"</b></p><p>Your offer value will be refunded succesfully to your wallet address <b><a href='"+process.env.WalletUrl+lastOfferData.address+"' target='_blank'>wallet address</a></b></p><p>Team OneTo11</p>";
...
async function nftOwnerOfferMail(nft,saleId, amount,offerer){
    ...
    if(typeof(nftData) == 'object' && typeof(sellerData)=='object' && typeof(offererData)=='object'){
        if(sellerData.notification && 'items' in (JSON.parse(sellerData.notification)) && (JSON.parse(sellerData.notification)).items==true){
            let htmlContent = "<p>Hi "+sellerData.displayName+",</p><p>Your listed NFT <b><a href='"+(JSON.parse(nftData.ipfs)).image+"' target='_blank'>"+(JSON.parse(nftData.ipfs)).name+"</a></b> received a new offer <b>"+Number(amount)/10**18+"</b> <img src='https://rest-api-oneto11-dev.herokuapp.com/currency/curr_logo.png' style='width:18px;height:18px; position:relative;top:-2px; margin-right:2px;' alt = ''/>. ";
            htmlContent = htmlContent + "<p>Offerer name is <b>"+offererData.displayName+"</b></p><p>Team OneTo11</p>";
