...
const contract_address = '0x57437ecE85dacd45E57369Ec8A2827508e4D526E'; //721
const abi = [
    {
        "inputs": [],
...
const MrketContractAdd = '0x043340240E925Fe94C86Dc0a51Ff09c50abf8DE4';
const marketAbi = [
    {
        "anonymous": false,
        "inputs": [
        ...
        if(count == 0){
            ...
            if(events){
                for(let i = 0; i < events.length; i++){
                    ...
                    let checkEntry = await transferModel.find({tokenId : list['tokenId']}).countDocuments();

                     if(checkEntry == 0 || checkEntry == null){
                           type = 'Mint';
                            let countCheck = await transferModel.find({from : list['from'],to : list['to'],txnHash : txs,tokenId : list['tokenId'],type : type,}).countDocuments();
                            if(countCheck == 0){
                            ...
                            if((list['to']).toUpperCase() == MrketContractAdd.toUpperCase()){
                                let market = await MarketContract.methods.checkMarket(list['tokenId']).call();
                                if(market == 'Auction'){
                                    type = 'Auction'
                                    let details = await MarketContract.methods.getAuction(list['tokenId']).call();
                                    price = details.price;
                                    console.log('auction price',price)
                                }else if(market == 'Sale'){
                                    type = 'Sale';
                                    price = await MarketContract.methods.getSellTokenPrice(list['tokenId']).call();
                                    console.log('sale price',price)
                                }else if(market == 'Deal'){
                                    type = 'Deal';
                                    price = await MarketContract.methods.dealDetailsByTokenId(list['tokenId']).call();
                                    console.log('sale price',price)
                                }
                            }else{
                                type = 'Transfer'
                            ...
                            const txnCount = await transferModel.find({txnHash : `${txs}`}).countDocuments();
