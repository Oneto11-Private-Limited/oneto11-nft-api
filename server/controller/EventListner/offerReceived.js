...
async function offerAcceptedMail(nftId,offerer,amount,gasPrice){
    ...
    if(typeof(nftData) == 'object' && typeof(offererData)=='object'){
        if(offererData.notification && 'items' in (JSON.parse(offererData.notification)) && (JSON.parse(offererData.notification)).items==true){
            ...
            htmlContent = htmlContent + "<p>Offer value: <b>"+amount/10**18+"</b><img src='https://rest-api-oneto11-dev.herokuapp.com/currency/curr_logo.png' style='width:18px;height:18px; position:relative;top:-2px; margin-right:2px;' alt = ''/>.</p><p>Platform fees <b>"+Number(PlatformFee)/10**18+"</b></p><p>Gas Fees <b>"+(gasPrice)/10**9+"</b></p><p>NFT successfully transferred to your wallet address. Please check and confirm.</p><p>Team OneTo11</p>";
...
async function nftSoldMail(nftId,amount,offerer,seller,gasPrice){
    ...
    if(typeof(nftData) == 'object' && typeof(sellerData)=='object'){
        if(sellerData.notification && 'items' in (JSON.parse(sellerData.notification)) && (JSON.parse(sellerData.notification)).items==true){
            ...
            htmlContent = htmlContent + "<p>Offer value: "+Number(amount)/10**18+"<img src='https://rest-api-oneto11-dev.herokuapp.com/currency/curr_logo.png' style='width:18px;height:18px; position:relative;top:-2px; margin-right:2px;' alt = ''/>.</p><p>Platform fees <b>"+Number(PlatformFee)/10**18+"</b></p><p>Gas Fees <b>"+(gasPrice)/10**9+"</b></p><p>NFT successfully transferred to purchaser <b><a href='"+process.env.WalletUrl+offerer+"' target='_blank'>wallet address</a></b></p><p>Team OneTo11</p>";
