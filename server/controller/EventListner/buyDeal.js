...
async function buyMail(nft, buyer, amount,txs){
    ...
    if(typeof(nftData) == 'object' && typeof(userData)=='object'){
        if(userData.notification && 'items' in (JSON.parse(userData.notification)) && (JSON.parse(userData.notification)).items==true){
            ...
            htmlContent = htmlContent + "<p>Price: <b>"+amount/10**18+"</b><img src='https://rest-api-oneto11-dev.herokuapp.com/currency/curr_logo.png' style='width:18px;height:18px; position:relative;top:-2px; margin-right:2px;' alt = ''/>.</p><p>Gas Fees <b>"+Number(gasPrice.gasPrice)/10**9+"</b></p><p>Team OneTo11</p>";
