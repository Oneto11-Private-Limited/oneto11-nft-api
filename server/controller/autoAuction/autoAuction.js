...
async function nftSoldMail(nftId,gaslimit,seller,lastBid,lastBidder){
    ...
    if(typeof(nftData) == 'object' && typeof(sellerData)=='object'){
        if(sellerData.notification && 'bids' in (JSON.parse(sellerData.notification)) && (JSON.parse(sellerData.notification)).bids==true){
            ...
            htmlContent = htmlContent + "<p>Bid value: <b>"+lastBid/10**18+"</b><img src='https://rest-api-oneto11-dev.herokuapp.com/currency/curr_logo.png' style='width:18px;height:18px; position:relative;top:-2px; margin-right:2px;' alt = ''/>.</p><p>Platform fees <b>"+Number(PlatformFee)/10**18+"</b></p><p>Gas Fees <b>"+gaslimit/10**18+"</b></p><p>NFT successfully transferred to purchaser <b><a href='"+process.env.WalletUrl+lastBidder+"' target='_blank'>wallet address</a></b></p><p>Team OneTo11</p>";
